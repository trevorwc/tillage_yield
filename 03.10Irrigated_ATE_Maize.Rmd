---
title: "Maize_ConstantTill_Irrigated_ATET"
author: "Trevor Cambron, adapted from Deines et al. 2019"
date: "8/23/2021"
output: html_document
---

# Causal Forests: Irrigated Long Term Tillage, Western US Cornbelt
...
In this analysis, the treatment is low-intensity tillage. The procedure is virtually idenfical to that used in 02.10Rainfed_ATE_Maize.Rmd
...
```{r}
dataFolder <- paste0('# Insert Folder')
...
scratchFolder <- paste0('# Insert Folder')
scratch2 <- paste0('# Insert Folder')
...
tillYears <- subset(tillYears, irrigation == 1)
drop <- c('irrigation')
tillYears <- tillYears[,!(names(tillYears) %in% drop)]
...
X_c1 <- overlap %>%
  dplyr::select(-c(pointID, latitude, longitude, tmin_jul, ppt_apr, fips, meanTemp_may, constTillStatus, count, nObs, W,tillStatus, uniqueID,silt,clay, nccpi3corn,
...
# This approach just adds JJA solar radiation, keeping the other variables limited to those idenified by MARS
varsToKeep = c(envvars,
                    'tc_def_jun','srad_jja', 
                    'slope', 'ppt_apr',  'vpd_july', 
                    'tc_soilm_apr', 'tc_def_jun','sand','soc', 'rootznaws')
```
...
Get ready to run causal forest, reload variables 
```{r}
# Causal Forest
# reload overlap
overlap <- readRDS(paste0(scratchFolder, '/overlap.rds'))
...
varsToKeep_main = c(envvars,
                    'tc_def_jun','srad_jja', 
                    'slope',  'vpd_july', 
                    'tc_soilm_apr', 'tc_def_jun','clay','soc', 'rootznaws')
...
tauhat_rf_aipw_t = c(ATE=atet_cf_aipw["estimate"],
                   ...
                   upper_ci=atet_cf_aipw["estimtate"] + 1.96 * atet_cf_aipw["std.err"])
print("Tons/ hectatre")
tauhat_rf_aipw_t
meanYield <- mean(overlapTau$Y)
...
With these outputs, we can both test for treatment effect heterogeneity as well as create a map of the treatment effects 
